...
from agent_skill_system import AgentSkillSystem
...
class AgentEconomicSystem:
    ...
    def run_simulation(self, agent_id: str):
        ...
        # ⭐ 스킬 학습 (NEW!)
        if 'skill_system' not in agent:
            agent['skill_system'] = AgentSkillSystem(agent_id)
        
        skill_learning_result = agent['skill_system'].learn_from_simulation(report)
        report['skills_learned'] = skill_learning_result
        
        # 레벨업 알림
        if skill_learning_result.get('level_ups'):
            print(f"\n🎉 스킬 레벨업 발생!")
            for level_up in skill_learning_result['level_ups']:
                print(f"   {level_up['skill_type'].upper()}: Level {level_up['old_level']} → {level_up['new_level']}")
        
        # NFT 발행 가능 알림
        mintable = skill_learning_result.get('can_mint_nft', [])
        if mintable:
            print(f"\n💎 {len(mintable)}개 스킬이 NFT 발행 가능합니다!")
            for item in mintable:
                skill = item['skill']
                print(f"   - {item['key']}: Level {skill['level']} ({skill['rarity']})")
    ...
    def export_to_json(self, agent_id: str, filepath: str):
        ...
        export_data = {
            ...
            'skill_system': agent.get('skill_system').to_dict() if 'skill_system' in agent else None
        }
