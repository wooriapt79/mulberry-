...
def main():
    ...
    # issue opened 이벤트는 comment가 없음 — issue body로 폴백
    comment_body = args.comment if args.comment.strip() else "(issue opened — no comment body)"

    mission = KodaMission(token=args.token)
    mission.validate_passport()
    result = mission.analyze_issue(args.issue, comment_body)
    logging.info(f"👤 Triggered by: {args.author or 'system'}")
